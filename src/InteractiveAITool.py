...
def on_predict_clicked(b):
    with output:
        ...
        print("=== Fermentation Status ===")
        print(f"Start (EST):  {start_dt.strftime('%Y-%m-%d %I:%M %p')}")
        print(f"Now (EST):    {now.strftime('%Y-%m-%d %I:%M %p')}")
        print(f"Elapsed real time: {elapsed_h:.1f} h"f" ({days_equiv_68:.1f} days)")
        print(f"Temperature: {temp_F:.1f}°F")
        print(f"Culture added: {'Yes' if has_culture else 'No'}\n")

        print(f"Estimated pH: {ph_est:.2f}")
        print(f"Estimated stage: {stage_now}\n")

        print("=== Upcoming Stage Changes ===")
        ...
        if has_culture and "curding" in future_map and "cheese" in future_map:
            cheese_t = future_map["cheese"]
            cur_t = future_map["curding"]
            if cur_t >= cheese_t:
                desired = cheese_t - 3.0 * 24.0  # aim ~3 days before cheese
                if desired <= hours_equiv_68:
                    desired = hours_equiv_68 + 1.0  # at least in the future
                if desired < cheese_t:
                    future_map["curding"] = desired
